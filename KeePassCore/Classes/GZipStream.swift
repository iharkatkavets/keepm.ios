...
public class GZipInputStream: InputStream {
        ...
        while remaining > 0 {
            if bufferOffset >= bufferSize {
                do {
                    ...
                    os_log(.error, log: outLog, "can't unzip error: %@", error as NSError)
                    break
                }
            }
        ...
        repeat {
            ...
            if inflateResult != Z_OK {
                ...
                if inflateResult != Z_STREAM_END {
                    os_log(.error, log: outLog, "inflate error %s", statusCodeToString(result: inflateResult))
                    throw GZipStreamError.failedToReadCompressedData
                }
    ...
    func statusCodeToString(result: Int32) -> String {
        switch result {
        case Z_OK:
            return "Z_OK"
        case Z_STREAM_END:
